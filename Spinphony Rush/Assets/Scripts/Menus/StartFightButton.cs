...
public class StartFightButton : MonoBehaviour
{
    ...
    public Dropdown selector1, selector2, selector3, selector4;
    ...
    private void openFightConfig()
    {
        ...
        if (tarjeta1.active == true)
        {
            PlayerPrefs.SetInt("controles1", selector1.value);
        }
        if (tarjeta2.active == true)
        {
            PlayerPrefs.SetInt("controles2", selector2.value);
        }
        if (tarjeta3.active == true)
        {
            PlayerPrefs.SetInt("controles3", selector3.value);
        }
        if (tarjeta4.active == true)
        {
            PlayerPrefs.SetInt("controles4", selector4.value);
        }
    }
    ...
    public int numPlayers()
    {
        ...
        if (tarjeta1.active == true) {
            players.Add(tarjeta1);
        }
        if (tarjeta2.active == true) {
            players.Add(tarjeta2);
        }
        if (tarjeta3.active == true) {
            players.Add(tarjeta3);
        }
        if (tarjeta4.active == true) {
            players.Add(tarjeta4);
        }
